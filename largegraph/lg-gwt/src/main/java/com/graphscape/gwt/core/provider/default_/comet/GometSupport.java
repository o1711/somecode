...
import com.graphscape.gwt.core.support.CollectionHandler;
...
public abstract class GometSupport implements GometI {
	...
	private UiLoggerI LOG = UiLoggerFactory.getLogger(GometSupport.class);
	...
	public void send(String jsS, HandlerI<String> onfailure) {
		if (LOG.isDebugEnabled()) {
			LOG.debug("sending, msg:" + jsS + ",onfailure:" + onfailure + ",comet:" + this);
		}
		this.sendInternal(jsS, onfailure);

	}

	protected abstract void sendInternal(String jsS, HandlerI<String> onfailure);
}
